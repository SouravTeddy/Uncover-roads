...
describe('buildReelCards', () => {
  it('wraps stops in intro + summary + stops + finale for single city', () => {
    const cards = buildReelCards(ITIN([STOP()]), null, null, new Map([['paris', WEATHER]]), 'explorer');
  ...
  it('injects a reco card at lunch window when no lunch stop exists', () => {
    ...
    const cards = buildReelCards(ITIN(stops), null, null, new Map([['paris', WEATHER]]), 'epicurean');
  ...
  it('inserts transit card between cities in multi-city journey', () => {
    ...
    const cards = buildReelCards(multiItin, legs, null, new Map([['paris', WEATHER]]), 'explorer');
  ...
  it('does not insert transit card when journeyLegs is empty', () => {
    ...
    const cards = buildReelCards(multiItin, [], null, new Map([['paris', WEATHER]]), 'explorer');
    const transit = cards.find(c => c.type === 'transit');
  ...
  it('uses pre-computed recos when recosByDayIdx is provided', () => {
    ...
    const cards = buildReelCards(ITIN(stops), null, null, new Map([['paris', WEATHER]]), 'explorer', recosByDayIdx);
  ...
  it('intel card with stopId is anchored to matching stop, not title-matched', () => {
    ...
    const cards = buildReelCards(itin, null, null, undefined, 'explorer');
  ...
  it('balance card message varies by category mix', () => {
    ...
    const cards = buildReelCards(itin, null, null, undefined, 'explorer', new Map([[0, []]]));
